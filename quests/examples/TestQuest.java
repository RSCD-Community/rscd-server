...
public class TestQuest extends Quest
{
	 ...
	 */
	/* See MiningQuest: ids below Quests.FIRST_CUSTOM belong to Jagex's list. */
	public final static int UID = 1001;
	...
	public void completeQuest()
	{
		...
		getOwner().getActionSender().sendMessage(getOwner().getNpc().getDef().getName() + " hands you 500 coins.");
		getOwner().incExp(12, 500, false); // 500 Crafting xp
					...
					public void handleReply(final int option, String response)
					{
						if(option == 0)
						{
							...
							} else
							{
								...
								owner.getInventory().remove(207, 10); // Remove the wool.
	...
	public void triggerEntity(QuestTrigger trigger, Entity entity)
	{
		...
		if(entity instanceof GameObject) // This entity is a GameObject.
		{
			...
			{
				getOwner().getActionSender().sendMessage(getName() + " - You acted on " + object.getGameObjectDef().getName());
			} else
			if(trigger == QuestTrigger.OBJECT_ACT2) // We acted upon command 2 of this object
			{
				getOwner().getActionSender().sendMessage(getName() + " - You acted2 on " + object.getGameObjectDef().getName());
			}
		}
	}
}